...
for k=1:size(SourcesMIPDECO,3)
    for j=1:size(SourcesMIPDECO,4)
        fig = figure(); clf;
        viewImage2Dsc(SourcesMIPDECO(:,:,k,j),domain,m);
        hold on;
        contour(XA',YA',W,1,'-r','LineWidth',3);
        caxis([0 1]);
        axis off
        
        mip = reshape(nnInter(SourcesMIPDECO(:,:,k,j),domain,xfine),size(W));
        intersect = mip.*W;
        union     = (mip+W)>eps;
...
for k=1:size(SourcesMIPDECO,3)
    for j=1:size(SourcesMIPDECO,4)
        fig = figure(); clf;
        viewImage2Dsc(InitMIPDECO(:,:,k,j),domain,m);
        hold on;
        contour(XA',YA',W,1,'-r','LineWidth',3);
        caxis([0 1]);
        
        mip = reshape(nnInter(InitMIPDECO(:,:,k,j),domain,xfine),size(W));
        intersect = mip.*W;
        union     = (mip+W)>eps;
        IoU = nnz(intersect)/nnz(union);
